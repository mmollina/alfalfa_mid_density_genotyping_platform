...
map.f1 <- map.construction(LG = LGs.f1[[ch]], 
                           expansion = ex, 
                           start.set = st, 
                           thres.hmm = thmm, 
                           phlim = phlim)
save(map.f1, dat.f1, ch,  file = paste0("~/repos/collaborations/bi-alfalfa-map/f1_map_construction/expansion_", ex, "_map_ch_", ch, ".rda"))
print(t2 <- Sys.time())
print(t2-t1)